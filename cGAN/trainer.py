...
class Trainer(object):
    ...
    def __init__(self, writer: SummaryWriter, train_loader: DataLoader, device: torch.device, args: dict, config: dict):
        ...
        self.generator = Generator(config=self.config['gen'],
                                   ...
                                   image_channels=config['image_channels']).to(self.device)
        ...
        if self.config['optimizer'] == 'Adam':
            self.optimizerG = optim.Adam(self.generator.parameters(), lr=config['learning_rate'], betas=(config['beta'], 0.999))
            self.optimizerD = optim.Adam(self.discriminator.parameters(), lr=config['learning_rate'], betas=(config['beta'], 0.999))
        elif self.config['optimizer'] == 'SGD':
            self.optimizerG = optim.SGD(self.generator.parameters(), lr=config['learning_rate'], momentum=0.9)
            self.optimizerD = optim.SGD(self.discriminator.parameters(), lr=config['learning_rate'], momentum=0.9)
