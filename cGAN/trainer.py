...
class Trainer(object):
    ...
    def __init__(self, writer: SummaryWriter, train_loader: DataLoader, device: torch.device, args: dict, config: dict):
        ...
        if self.config['optimizer'] == 'Adam':
            ...
            self.optimizerG = optim.NAdam(self.generator.parameters(), lr=self.config['learning_rate'], betas=(self.config['beta'], 0.999))
            self.optimizerD = optim.NAdam(self.discriminator.parameters(), lr=self.config['learning_rate'], betas=(self.config['beta'], 0.999))
        else:
